 ...
 */
public class DynamicPatternStream<T> {

	private final DynamicPatternStreamBuilder<T> builder;

	private DynamicPatternStream(final DynamicPatternStreamBuilder<T> builder) {
		this.builder = checkNotNull(builder);
	}

	DynamicPatternStream(final DataStream<T> inputStream, final DataStream<Tuple2<String, Pattern<T, ?>>> pattern) {
		this(DynamicPatternStreamBuilder.forStreamAndPattern(inputStream, pattern));
	}

	DynamicPatternStream<T> withComparator(final EventComparator<T> comparator) {
		return new DynamicPatternStream<>(builder.withComparator(comparator));
	}

	public DynamicPatternStream<T> sideOutputLateData(OutputTag<T> lateDataOutputTag) {
		return new DynamicPatternStream<>(builder.withLateDataOutputTag(lateDataOutputTag));
	}
	 ...
	 * @param <R> Type of the resulting elements
	 *
	 * @deprecated Use {@link DynamicPatternStream#select(OutputTag, PatternTimeoutFunction, PatternSelectFunction)}
	 ...
	 *
	 * @deprecated Use {@link DynamicPatternStream#flatSelect(OutputTag, PatternFlatTimeoutFunction, PatternFlatSelectFunction)}
	 * that returns timed out events as a side-output
	 *
