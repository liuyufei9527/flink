...
import org.apache.flink.cep.operator.DynamicCepOperator;
 ...
 */
@Internal
final class DynamicPatternStreamBuilder<IN> {

	private final DataStream<IN> inputStream;

	private final DataStream<Tuple2<String, Pattern<IN, ?>>> patternInputStream;
	...
	private final String OPERATOR_NAME = "dynamicCepOperator";

	private DynamicPatternStreamBuilder(
		final DataStream<IN> inputStream,
		final DataStream<Tuple2<String, Pattern<IN, ?>>> patternInputStream,
		@Nullable final EventComparator<IN> comparator,
		@Nullable final OutputTag<IN> lateDataOutputTag) {

		this.inputStream = checkNotNull(inputStream);
		this.patternInputStream = checkNotNull(patternInputStream);
	...
	DynamicPatternStreamBuilder<IN> withComparator(final EventComparator<IN> comparator) {
		return new DynamicPatternStreamBuilder<>(inputStream, patternInputStream, checkNotNull(comparator), lateDataOutputTag);
	}

	DynamicPatternStreamBuilder<IN> withLateDataOutputTag(final OutputTag<IN> lateDataOutputTag) {
		return new DynamicPatternStreamBuilder<>(inputStream, patternInputStream, comparator, checkNotNull(lateDataOutputTag));
	}
	...
	<OUT, K> SingleOutputStreamOperator<OUT> build(
		...
		final DynamicCepOperator<IN, K, OUT> operator = new DynamicCepOperator<>(
			inputSerializer,
			isProcessingTime,
			comparator,
			processFunction,
		...
		BroadcastStream<Tuple2<String, Pattern<IN, ?>>> broadcastStream = patternInputStream.broadcast(patternBroadcastStateDescriptors);

		final SingleOutputStreamOperator<OUT> patternStream;
		if (inputStream instanceof KeyedStream) {
			KeyedStream<IN, K> keyedStream = (KeyedStream<IN, K>) inputStream;

			patternStream = keyedStream.connect(broadcastStream).transform(
				"",
				outTypeInfo,
				operator
			);
			...
			patternStream = inputStream.keyBy(keySelector).connect(broadcastStream).transform(
				"dynamicCepOperator",
				outTypeInfo,
				operator
			).forceNonParallel();
	...
	static <IN> DynamicPatternStreamBuilder<IN> forStreamAndPattern(final DataStream<IN> inputStream, DataStream<Tuple2<String, Pattern<IN, ?>>> patternInputStream) {
		return new DynamicPatternStreamBuilder<>(inputStream, patternInputStream, null, null);
	}
}
