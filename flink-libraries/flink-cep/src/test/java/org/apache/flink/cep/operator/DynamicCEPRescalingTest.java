...
import static org.apache.flink.cep.operator.CepOperatorTestUtilities.getDynamicKeyedCepOpearator;
 ...
 */
public class DynamicCEPRescalingTest {
	...
	public void testCEPFunctionScalingUp() throws Exception {
		...
		KeyedTwoInputStreamOperatorTestHarness<Integer, Event, Tuple2<String, Pattern<Event, ?>>, Map<String, List<Event>>> harness = null;
		KeyedTwoInputStreamOperatorTestHarness<Integer, Event, Tuple2<String, Pattern<Event, ?>>, Map<String, List<Event>>> harness1 = null;
		KeyedTwoInputStreamOperatorTestHarness<Integer, Event, Tuple2<String, Pattern<Event, ?>>, Map<String, List<Event>>> harness2 = null;

		try {
			harness = getTestHarness(maxParallelism, 1, 0);
			harness.open();

			Tuple2<String, Pattern<Event, ?>> testPattern = Tuple2.of("testPattern", getPattern());
			harness.processElement2(new StreamRecord<>(testPattern, 1));

			harness.processElement1(
				new StreamRecord<>(startEvent1, 1));                        // valid element
			harness.processElement1(new StreamRecord<>(new Event(7, "foobar", 1.0), 2));
			...
			harness1.processBothWatermarks(new Watermark(2));

			harness1.processElement1(
				new StreamRecord<Event>(middleEvent1, 3));                // valid element
			harness1.processElement1(
				new StreamRecord<>(endEvent1, 5));                        // valid element

			harness1.processBothWatermarks(new Watermark(Long.MAX_VALUE));
			...
			harness2.processBothWatermarks(new Watermark(2));

			harness2.processElement1(new StreamRecord<>(endEvent2, 5));
			harness2.processElement1(new StreamRecord<>(new Event(42, "start", 1.0), 4));

			harness2.processBothWatermarks(new Watermark(Long.MAX_VALUE));
	...
	private static void closeSilently(KeyedTwoInputStreamOperatorTestHarness<?, ?, ?, ?> harness) {
		if (harness != null) {
			try {
				harness.close();
	...
	public void testCEPFunctionScalingDown() throws Exception {
		...
		KeyedTwoInputStreamOperatorTestHarness<Integer, Event, Tuple2<String, Pattern<Event, ?>>, Map<String, List<Event>>> harness1 =
			getTestHarness(maxParallelism, 3, 0);
		harness1.open();

		KeyedTwoInputStreamOperatorTestHarness<Integer, Event, Tuple2<String, Pattern<Event, ?>>, Map<String, List<Event>>> harness2 =
			getTestHarness(maxParallelism, 3, 1);
		harness2.open();

		KeyedTwoInputStreamOperatorTestHarness<Integer, Event, Tuple2<String, Pattern<Event, ?>>, Map<String, List<Event>>> harness3 =
			getTestHarness(maxParallelism, 3, 2);
		harness3.open();

		KeyedTwoInputStreamOperatorTestHarness<Integer, Event, Tuple2<String, Pattern<Event, ?>>, Map<String, List<Event>>> harness4 = null;
		KeyedTwoInputStreamOperatorTestHarness<Integer, Event, Tuple2<String, Pattern<Event, ?>>, Map<String, List<Event>>> harness5 = null;

		try {
			harness1.processWatermark1(new Watermark(Long.MIN_VALUE));
			harness2.processWatermark1(new Watermark(Long.MIN_VALUE));
			harness3.processWatermark1(new Watermark(Long.MIN_VALUE));

			harness1.processElement1(
				new StreamRecord<>(startEvent1, 1));                        // valid element
			harness1.processElement1(new StreamRecord<>(new Event(7, "foobar", 1.0), 2));
			harness1.processElement1(
				new StreamRecord<Event>(middleEvent1, 3));                    // valid element
			harness1.processElement1(
				new StreamRecord<>(endEvent1, 5));                            // valid element
			...
			harness1.processElement1(new StreamRecord<>(startEvent3, 10));
			harness1.processElement1(new StreamRecord<>(startEvent1, 10));

			harness2.processElement1(new StreamRecord<>(startEvent2, 7));
			harness2.processElement1(new StreamRecord<Event>(middleEvent2, 8));

			harness3.processElement1(new StreamRecord<>(startEvent4, 15));
			harness3.processElement1(new StreamRecord<Event>(middleEvent4, 16));
			harness3.processElement1(new StreamRecord<>(endEvent4, 17));
			...
			harness5.processElement1(new StreamRecord<>(endEvent2, 11));
			harness5.processWatermark1(new Watermark(12));
			...
			// if element timestamps are not correctly checkpointed/restored this will lead to
			// a pruning time underflow exception in NFA
			harness4.processWatermark1(new Watermark(12));
			...
			harness4.processWatermark1(new Watermark(Long.MAX_VALUE));
			harness5.processWatermark1(new Watermark(Long.MAX_VALUE));
	...
	private KeyedTwoInputStreamOperatorTestHarness<Integer, Event, Tuple2<String, Pattern<Event, ?>>, Map<String, List<Event>>> getTestHarness(
			int maxParallelism,
			int taskParallelism,
			int subtaskIdx) throws Exception {

		KeySelector<Event, Integer> keySelector = new TestKeySelector();
		KeyedTwoInputStreamOperatorTestHarness<Integer, Event, Tuple2<String, Pattern<Event, ?>>, Map<String, List<Event>>> harness =
				new KeyedTwoInputStreamOperatorTestHarness<>(
					getDynamicKeyedCepOpearator(
								false),
						keySelector,
					null,
	...
	private static Pattern<Event, ?> getPattern() {
